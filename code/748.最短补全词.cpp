 ...
 *
 * 请返回 words 中的 最短补全词
 * 。题目数据保证一定存在一个最短补全词。当有多个单词都符合最短补全词的匹配条件时取 words 中 第一个
 * 出现的那个。
 *
 *
 *
...
class Solution {
  public:
    string shortestCompletingWord(string licensePlate, vector<string>& words) {
        ...
        for (auto word : words) {
            if (word.size() < minlen) {
                unordered_map<char, int> frq1;
                for (auto c : word) frq1[c]++;
                if (ok(frq, frq1)) {
                    minlen = word.size();
                    ans = word;
                }
    ...
    bool ok(unordered_map<char, int> frq0, unordered_map<char, int> frq1) {
        for (auto [c, t] : frq0) {
            if (frq1[c] < t) return false;
        }
        return true;
    }
};
